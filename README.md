...
Four coordinated AI agents run in sequence via **Google ADK**:

```
Field Photo
    │
    ▼
1. Capture Agent   (Gemini 2.5 Flash Vision) → FindingDraft
    │
    ▼
2. Analyze Agent   (RAG + MCP + Gemini 2.5 Flash) → RegulatoryCheck
    │
    ▼
3. Report Agent    (Gemini 2.5 Flash) → ReportSection / FullReport
    │
    ▼
4. Audit Agent     (Gemini 2.5 Flash) → statute verification + badge
    │
    ▼
...
| **AuditAgent** | Gemini 2.5 Flash | `FullReport` → verified statute count + compliance badge |
...
| | |
|---|---|
| **Health check** | https://inspectiq-agent-259799500638.us-central1.run.app/health |
| **HTML Report** | https://inspectiq-agent-259799500638.us-central1.run.app/demo-report |
| **Interactive API docs** | https://inspectiq-agent-259799500638.us-central1.run.app/docs |
| **Demo JSON** | https://inspectiq-agent-259799500638.us-central1.run.app/demo-result |
| **Web UI** | https://inspectiq-agent-259799500638.us-central1.run.app/inspect-ui |
...
| `POST` | `/adk-pipeline` | Full 4-agent ADK SequentialAgent with MCP |
| `GET` | `/report/{id}` | HTML report by UUID |
| `GET` | `/inspect-ui` | Web UI — paste photo URL, get report |
...
| Regulatory analysis + narrative | Gemini 2.5 Flash (Vertex AI ADC) |
| Vector store (FL regulations) | ChromaDB |
| Regulatory tool server | MCP (FastMCP stdio server) |
| HTTP API | FastAPI + Uvicorn |
| Production deployment | Google Cloud Run (us-central1) |
| Runtime | Python 3.11 |
